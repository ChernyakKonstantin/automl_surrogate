...
class HeteroPipelineAndDatasetFeaturesDataset(Dataset):
    def __init__(
        ...
        self.metrics = self.task_pipe_comb.groupby("dataset_id").apply(lambda x: x.metric.unique())
    ...
    def _get_pipeline_path(self, dataset_id: str, x: pd.Series) -> str:
            return os.path.join(self.root, self.pipelines_dir, dataset_id, x["architecture_id"], x["hparam_id"] + ".pickle")
    ...
    def _get_sample(self) -> Tuple[Sequence[HeterogeneousData], Tensor, Sequence[float]]:
        ...
        metrics = self.metrics.loc[dataset_id]

        selected_metrics = np.random.choice(metrics, self.pipelines_per_step, replace=False)

        group = self.task_pipe_comb.loc[dataset_id]
        group = group.merge(pd.DataFrame(data=selected_metrics, columns=["metric"]), on="metric", how="inner")

        idxes = group.groupby("metric").apply(lambda x: np.random.choice(x.index, 1)[0])

        samples = group.loc[idxes]

        # pipeline_paths = samples.apply(lambda x: self._get_pipeline_path(dataset_id, x), axis=1).to_list()
